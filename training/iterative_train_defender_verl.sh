#!/bin/bash
# Defender GRPO training on PrivacyPeek cases (Path B).
...
SAVE_PREFIX=${SAVE_PREFIX:-privacy_defender_privacypeek_${RUN_TIMESTAMP}}
ROUNDS=${ROUNDS:-1}
...
DATA_OUTPUT_DIR=${DATA_OUTPUT_DIR:-$PROJECT_DIR/data/privacypeek}
CONFIG_PATH=${CONFIG_PATH:-$PROJECT_DIR/examples/config_privacypeek.yaml}
REWARD_FN=${REWARD_FN:-$PROJECT_DIR/reward/privacypeek_reward_wrapper.py:compute_score}
...
echo "[INFO] PrivacyPeek defender training"
...
for ((round=0; round<ROUNDS; round++)); do
  echo "===== Defender round $round/$ROUNDS ====="
  ...
  echo "[DATA] Building PrivacyPeek dataset..."
  python3 "$PROJECT_DIR/build_privacypeek_dataset.py" "${BUILD_ARGS[@]}"
...
echo "PrivacyPeek defender training completed!"
