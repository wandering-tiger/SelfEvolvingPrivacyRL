#!/bin/bash
# PrivacyPeek defender smoke/full train — uses all 5 GPUs.
#
# Layout (hardcoded, ignore leftover shell env for GPU ids):
#   GPU 0,1,3,4 → veRL FSDP actor + rollout vLLM TP=4
#   GPU 2       → guard vLLM (reward / PrivacyPeek env)
#
# Usage:
#   bash scripts/run_privacypeek_5gpu.sh                 # smoke, 20 cases
#   bash scripts/run_privacypeek_5gpu.sh full             # full run
#   bash scripts/run_privacypeek_5gpu.sh smoke 50         # smoke with limit=50
...
# Fresh defaults — clear any leftover TRAIN_GPUS=4 from earlier sessions.
...
bash "$PROJECT_DIR/scripts/guard_privacypeek_train.sh" "$BASE_MODEL" "$RUN_ID"
